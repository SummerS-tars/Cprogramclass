...
{
    /* 地图信息读入 */
    int openRight = mapInput( mapInfo , customMapOrder + 3 ) ;      // TODO:新增地图打开检测
    if( !openRight )
    {
        system("cls") ;
        printf("地图文件打开失败\n") ;
...
extern void mapCustomIni( int customMapOrder , int priInfo )    
{
    ...
    progressSaveInit( customMapOrder + 3 ) ; // 同时初始化游戏存档  // TODO:BUG7修复记录:调整了调用参数，解决存档初始化失效的问题
    return ;
}
...
extern void mapCustomEdit( int customMapOrder , int loadFlag )   
{
    ...
    if( loadFlag )
    {
        progressSaveInit( customMapOrder + 3 ) ; // 初始化游戏存档  // TODO:BUG7修复记录:增加存档初始化，解决编辑已游玩地图时存档初始化问题
        editProgressLoad( &mapInfo , customMapOrder , &Tnum , &Dnum , &playerSetFlag ) ;
    }
    else
    {   // 初始化
