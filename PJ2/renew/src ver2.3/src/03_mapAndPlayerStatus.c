#include "01_menuUpdate.h"  // TODO:2.所有有关mapPath和savePath的地方都要修改,通过修改mapOrder来实现 finished
...
{
    FILE *savefp = NULL;
    savefp = fopen(savePath[mapOrder - 1], "r");
    if( savefp == NULL )
    {
        printf("进度保存文件打开失败！\n") ;
        return ;
...
void saveOperation( struct playerInfo *player , struct mapInfo *mapInfo , int mapOrder )    // 保存游戏进度
{
    /* 打开存档文件写入 */
    FILE *savefp = NULL ;
    savefp = fopen( savePath[ mapOrder - 1 ] , "w" ) ;
    if( savefp == NULL )
    {
        printf("进度保存文件打开失败！\n") ;
        return ;
...
void progressSaveInit( int mapOrder )    // TODO:2.已经修改mapOrder引用
{
    FILE *savefp = NULL ;
    savefp = fopen( savePath[ mapOrder - 1 ] , "w" ) ;
    if( savefp == NULL )
