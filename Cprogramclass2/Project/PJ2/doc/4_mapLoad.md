...
1. `地图选择菜单`更改
   ...
   2. 根据`mapnum`确定输出选项的数量(总共有`mapnum`+2(`新建地图`和`返回上级菜单`个选项))  
      1. 选项打印需要新的菜单专属逻辑  
      2. 选项选中则仍然可以沿用统一逻辑  
   3. 地图选项采用`<mapName>`的方式打出  
      1. 选中之后直接接入`地图统一操作菜单`(设想`地图统一操作菜单`由`customOperationMenu`修改而来)  
   4. `新建地图`选项
...
2. 其他更改  
   ...
   2. 尽量维持`mapName`调用逻辑不变的前提下,使其实际包含选项数量可以宽松化
   3. 由于`mapName`可以变化,那么我还要对应变化`mapProgressSave`
   4. 给`地图统一操作菜单`增加重命名功能(如果还有时间的话加)